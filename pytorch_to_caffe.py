...
def _conv2d(
    ...
    layer = caffe_net.Layer_param(
        name=name, type="Convolution", bottom=[log.blobs(input)], top=[log.blobs(x)]
    )
    layer.conv_param(
        x.size()[1],
        weight.size()[2:],
...
def _conv_transpose2d(
    ...
    layer = caffe_net.Layer_param(
        name=name, type="Deconvolution", bottom=[log.blobs(input)], top=[log.blobs(x)]
    )
    layer.conv_param(
        x.size()[1],
        weight.size()[2:],
...
def _split(raw, tensor, split_size, dim=0, torch_name=None):
    ...
    layer = caffe_net.Layer_param(
        name=layer_name, type="Slice", bottom=[log.blobs(tensor)], top=[log.blobs(x)]
    )
...
def _pool(
    type, raw, input, x, kernel_size, stride, padding, ceil_mode, torch_name=None
):
    ...
    layer = caffe_net.Layer_param(
        name=layer_name, type="Pooling", bottom=[log.blobs(input)], top=[log.blobs(x)]
    )
    ...
    if ceil_mode == False and stride is not None:
        oheight = (input.size()[2] - _pair(kernel_size)[0] + 2 * _pair(padding)[0]) % (
            _pair(stride)[0]
        )
        owidth = (input.size()[3] - _pair(kernel_size)[1] + 2 * _pair(padding)[1]) % (
            _pair(stride)[1]
        )
...
def _max(raw, *args, torch_name=None):
    ...
    if len(args) == 1:
        ...
        layer = caffe_net.Layer_param(
            name=layer_name, type="Eltwise", bottom=bottom_blobs, top=[log.blobs(x)]
        )
...
def _cat(raw, inputs, dimension=0, torch_name=None):
    ...
    layer = caffe_net.Layer_param(
        name=layer_name, type="Concat", bottom=bottom_blobs, top=[log.blobs(x)]
    )
...
def _dropout(raw, input, p=0.5, training=False, inplace=False, torch_name=None):
    ...
    layer = caffe_net.Layer_param(
        name=layer_name, type="Dropout", bottom=bottom_blobs, top=[log.blobs(x)]
    )
    layer.param.dropout_param.dropout_ratio = p
    layer.param.include.extend(
        [caffe_net.pb.NetStateRule(phase=0)]
...
def _threshold(raw, input, threshold, value, inplace=False, torch_name=None):
    ...
    layer = caffe_net.Layer_param(
        name=layer_name, type="Threshold", bottom=bottom_blobs, top=[log.blobs(x)]
    )
...
def _batch_norm(
    ...
    layer1 = caffe_net.Layer_param(
        name=layer_name1, type="BatchNorm", bottom=bottom_blobs, top=[log.blobs(x)]
    )
    ...
    if weight is not None and bias is not None:
        ...
        layer2 = caffe_net.Layer_param(
            name=layer_name2, type="Scale", bottom=[log.blobs(x)], top=[log.blobs(x)]
        )
...
def _instance_norm(
    ...
    if weight is not None and bias is not None:
        layer_name2 = log.add_layer(name="bn_scale", torch_name=torch_name)
        layer2 = caffe_net.Layer_param(
            name=layer_name2, type="Scale", bottom=top_blobs, top=[log.blobs(x)]
        )
        layer2.param.scale_param.bias_term = True
        layer2.add_data(weight.cpu().data.numpy(), bias.cpu().data.numpy())
        log.cnet.add_layer(layer2)
...
def _interpolate(
    ...
    layer = caffe_net.Layer_param(
        name=layer_name, type="Upsample", bottom=[log.blobs(input)], top=[log.blobs(x)]
    )
...
def _squeeze(raw, inputs, *args, torch_name=None):
    ...
    layer = caffe_net.Layer_param(
        name=layer_name, type="Reshape", bottom=[log.blobs(inputs)], top=[log.blobs(x)]
    )
    dims = [0, -1]
...
def _view(input, *args, torch_name=None):
    ...
    top_blobs = log.add_blobs([x], name="view_blob")
    layer = caffe_net.Layer_param(
        name=layer_name, type="Reshape", bottom=[log.blobs(input)], top=[log.blobs(x)]
    )
...
def _mean(input, *args, torch_name=None, **kwargs):
    ...
    layer = caffe_net.Layer_param(
        name=layer_name, type="Reduction", bottom=[log.blobs(input)], top=[log.blobs(x)]
    )
    if len(args) == 1:
        dim = args[0]
    elif "dim" in kwargs:
...
def _add(input, *args, torch_name=None):
    ...
    if isinstance(args[0], (int, float)):
        ...
        layer = caffe_net.Layer_param(
            name=layer_name, type="Scale", bottom=[log.blobs(input)], top=[log.blobs(x)]
        )
...
def _iadd(input, *args, torch_name=None):
    ...
    layer = caffe_net.Layer_param(
        name=layer_name, type="Eltwise", bottom=[b1, b2], top=[log.blobs(x)]
    )
    layer.param.eltwise_param.operation = 1  # sum is 1
    log.cnet.add_layer(layer)
    return x
...
def _sub(input, *args, torch_name=None):
    ...
    layer.param.eltwise_param.operation = 1  # sum is 1
    layer.param.eltwise_param.coeff.extend([1.0, -1.0])
    log.cnet.add_layer(layer)
    return x
