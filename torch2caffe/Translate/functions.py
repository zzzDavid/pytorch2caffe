...
def _split(translog, raw, tensor, split_size, dim=0, torch_name=None):
    ...
    layer = caffe_net.Layer_param(
        name=layer_name,
        type="Slice",
        bottom=[translog.blobs[id(tensor)]],
        top=[translog.blobs[id(x)]],
    )
...
def _cat(translog, raw, inputs, dimension=0, torch_name=None):
    ...
    layer = caffe_net.Layer_param(
        name=layer_name, type="Concat", bottom=bottom_blobs, top=[translog.blobs[id(x)]]
    )
...
def _dropout(
    translog, raw, input, p=0.5, training=False, inplace=False, torch_name=None
):
...
def _threshold(translog, raw, input, threshold, value, inplace=False, torch_name=None):
    ...
    if threshold == 0 and value == 0:
        ...
        layer = caffe_net.Layer_param(
            name=name, type="ReLU", bottom=bottom_blobs, top=[translog.blobs[id(x)]]
        )
        translog.cnet.add_layer(layer)
        return x
    if value != 0:
...
def _prelu(translog, raw, input, weight, torch_name=None):
    ...
    layer = caffe_net.Layer_param(
        name=name, type="PReLU", bottom=bottom_blobs, top=[translog.blobs[id(x)]]
    )
    if weight.size()[0] == 1:
...
def _leaky_relu(
    translog, raw, input, negative_slope=0.01, inplace=False, torch_name=None
):
...
def _softmax(translog, raw, input, dim=None, _stacklevel=3, torch_name=None):
    ...
    layer = caffe_net.Layer_param(
        name=name, type="Softmax", bottom=bottom_blobs, top=[translog.blobs[id(x)]]
    )
...
def _batch_norm(
    ...
    logger.info(
        f"---> layer name: {layer_name1}, bottom blobs: {bottom_blobs}, top blobs: {top_blobs}"
    )
    ...
    if weight is not None and bias is not None:
        ...
        logger.info(
            f"---> layer name: {layer_name2}, bottom blobs: {[translog.blobs[id(x)]]}, top blobs: {[translog.blobs[id(x)]]}"
        )
        layer2 = caffe_net.Layer_param(
            name=layer_name2,
            type="Scale",
            bottom=[translog.blobs[id(x)]],
            top=[translog.blobs[id(x)]],
        )
...
def _squeeze(translog, raw, inputs, *args, torch_name=None):
    ...
    layer = caffe_net.Layer_param(
        name=layer_name,
        type="Reshape",
        bottom=[translog.blobs[id(inputs)]],
        top=[translog.blobs[id(x)]],
    )
    dims = [0, -1]
...
def _flatten(translog, raw, inputs, *args, torch_name=None):
    ...
    logger.info(
        f"---> layer name: {layer_name}, bottom blobs: {[translog.blobs[id(inputs)]]}, top blobs: {top_blobs}"
    )
    layer = caffe_net.Layer_param(
        name=layer_name,
        type="Flatten",
    ...
    def __patched_add__(input, *args):
        ...
        if isinstance(args[0], (int, float)):
            ...
            layer = caffe_net.Layer_param(
                name=layer_name,
                type="Eltwise",
                bottom=[translog.blobs[id(input)], translog.blobs[id(args[0])]],
                top=[translog.blobs[id(x)]],
            )
...
def _iadd(raw, translog):
    def __patched_iadd__(input, other):
        if (
            id(input) not in translog.blobs.keys()
            and id(other) not in translog.blobs.keys()
        ):
            return input
        ...
        logger.info(
            f"---> layer name: {layer_name}, bottom blobs: {[b1, b2]}, top blobs: {top_blobs}"
        )
        layer_param = caffe_net.Layer_param(
            name=layer_name, type="Eltwise", bottom=[b1, b2], top=top_blobs
        )
...
def _mul(raw, translog):
    def __patched_mul__(input, *args):
        ...
        if not (
            args[0].shape[2] == input.shape[2] and args[0].shape[3] == input.shape[3]
        ):
            print(
            ...
            layer = caffe_net.Layer_param(
                name=layer_name,
                type="Scale",
                bottom=[translog.blobs[id(input)], translog.blobs[id(y)]],
                top=[translog.blobs[id(x)]],
            )
            layer.param.scale_param.bias_term = False
